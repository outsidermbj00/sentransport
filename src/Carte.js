...
// Calculer la distance entre 2 points GPS (km) via la formule de Haversine
...
const d = calculerDistance(positionUtilisateur[0], positionUtilisateur[1], a[" lat "], a[" lon "]);
            ...
            console.log("Arrêt le plus proche trouvé :", proche);
    ...
    return (
        <div className="carte-container">
            ...
            <MapContainer center={DAKAR} zoom={13} className="carte">
                ...
                {/* 1. Sécuriser les marqueurs des arrêts */}
                {arrets.map(a => {
                    // SÉCURITÉ : On vérifie que l'arrêt possède bien une latitude et une longitude valides
                    if (a.lat === undefined || a.lon === undefined || a.lat === null || a.lon === null) {
                        console.warn(`L'arrêt avec l'ID ${a.id} a des coordonnées invalides`, a);
                        return null; // On ignore cet arrêt pour éviter le crash
                    }
                ...
                {/* 2. Sécuriser le marqueur de l'utilisateur */}
